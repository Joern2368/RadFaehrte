...
struct RadFaehrteTests {
    ...
    @Test func routeRepositoryFindsEuroVeloRouteNearAthens() async throws {
        // Regressionstest für die zusätzliche Griechenland-Datenbank (greece.sqlite, s.
        // ROADMAP.md) - analog zu den Niederlande-/Polen-/Schweden-/Dänemark-/Belgien-/
        // Luxemburg-/Schweiz-/Frankreich-/Österreich-/Tschechien-/Slowakei-/Albanien-/
        // Italien-/Spanien-/Portugal-/Malta-/Andorra-/Liechtenstein-/Nordmazedonien-/Kosovo-/
        // Montenegro-/Bosnien-und-Herzegowina-/Serbien-/Kroatien-/Slowenien-/Bulgarien-/
        // Ungarn-/Rumänien-Tests oben.
        let repository = RouteRepository()

        let routes = repository.routesOverlapping(
            minLon: 23.65, minLat: 37.90, maxLon: 23.80, maxLat: 38.05
        )
    ...
    @Test func routeRepositoryFindsEuroVeloRouteNearNicosia() async throws {
        // Regressionstest für die zusätzliche Zypern-Datenbank (cyprus.sqlite, s.
        // ROADMAP.md) - analog zu den Niederlande-/Polen-/Schweden-/Dänemark-/Belgien-/
        // Luxemburg-/Schweiz-/Frankreich-/Österreich-/Tschechien-/Slowakei-/Albanien-/
        // Italien-/Spanien-/Portugal-/Malta-/Andorra-/Liechtenstein-/Nordmazedonien-/Kosovo-/
        // Montenegro-/Bosnien-und-Herzegowina-/Serbien-/Kroatien-/Slowenien-/Bulgarien-/
        // Ungarn-/Rumänien-/Griechenland-Tests oben.
        let repository = RouteRepository()

        let routes = repository.routesOverlapping(
            minLon: 33.30, minLat: 35.10, maxLon: 33.40, maxLat: 35.20
        )

        #expect(!routes.isEmpty)
        #expect(routes.contains { $0.name?.contains("EuroVelo 8") == true })

        let euroVelo8Cyprus = routes.first { $0.name?.contains("EuroVelo 8") == true }
    ...
    @Test func routeRepositoryFindsEuroVeloRouteNearOslo() async throws {
        // Regressionstest für die zusätzliche Norwegen-Datenbank (norway.sqlite, s.
        // ROADMAP.md) - analog zu den Niederlande-/Polen-/.../Island-/Irland-/Finnland-Tests
        // oben.
        let repository = RouteRepository()

        let routes = repository.routesOverlapping(
            minLon: 10.70, minLat: 59.88, maxLon: 10.80, maxLat: 59.93
        )

        #expect(!routes.isEmpty)
        #expect(routes.contains { $0.name?.contains("EuroVelo 3") == true })

        let euroVelo3Norway = routes.first { $0.name?.contains("EuroVelo 3") == true }
        #expect(euroVelo3Norway?.lines.isEmpty == false)
    }

    @Test func routeRepositoryFindsEuroVeloRouteNearLondon() async throws {
        // Regressionstest für die zusätzliche Großbritannien-Datenbank (great-britain.sqlite, s.
        // ROADMAP.md) - analog zu den Niederlande-/Polen-/.../Finnland-/Norwegen-Tests oben.
        let repository = RouteRepository()

        let routes = repository.routesOverlapping(
            minLon: -0.18, minLat: 51.47, maxLon: -0.07, maxLat: 51.53
        )

        #expect(!routes.isEmpty)
        #expect(routes.contains { $0.name?.contains("EuroVelo 2") == true })

        let euroVelo2London = routes.first { $0.name?.contains("EuroVelo 2") == true }
        #expect(euroVelo2London?.lines.isEmpty == false)
    }
