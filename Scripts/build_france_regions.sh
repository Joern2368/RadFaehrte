#!/bin/bash
# Baut die Offline-Wege-Graphen (Format v2) für die 21 französischen Regionen (Geofabrik-
# Einteilung vor der 2016er-Reform) einzeln statt der einen zu großen Gesamt-Frankreich-Datei
# (4,7 GB) - die scheiterte zweimal am Speicherdruck des 16-GB-Baurechners (s. ROADMAP.md).
# Jede Region liegt zwischen 32 MB (Corse) und 500 MB (Rhône-Alpes), also in der bereits
# bewährten Größenordnung (vgl. Dänemark 469 MB, Schweiz 516 MB).
set -uo pipefail
cd "$(dirname "$0")/.."
...
SUMMARY="Scripts/data/france_regions_summary.txt"
> "$SUMMARY"
...
for slug in $REGIONS; do
  ...
  EXPECTED=$(curl -sIL --fail "https://download.geofabrik.de/europe/france/${slug}-latest.osm.pbf" | grep -i content-length | tail -1 | tr -d '\r' | awk '{print $2}')
  if ! curl -L --fail --retry 3 -o "Scripts/data/${slug}-latest.osm.pbf" "https://download.geofabrik.de/europe/france/${slug}-latest.osm.pbf"; then
...
" 2>/dev/null)
  ...
  echo "=== $slug: uploading to way-graphs-fr-v1 ==="
  if ! gh release upload way-graphs-fr-v1 "Scripts/data/${slug}_ways.sqlite" --repo Joern2368/RadFaehrte --clobber; then
